 ...
 */
class SmartySessionHelper extends Helper {
 ...
 */
	function _register_smarty_functions(&$smarty) {
		$smarty->register_function('session', array(&$this, 'session'));
	}

/**
 * Smarty wrapper for SessionHelper
 *
 * @param mixed $params params from Smarty template call
 * @param Smarty $smarty Smarty object
 * @return mixed
 */
	function session($params, &$smarty) {
		// sanity check for php version
		if (!class_exists('ReflectionClass')) {
			$smarty->trigger_error("SmartySession: Error - requires php 5.0 or above", E_USER_NOTICE);
			return;
		}
		...
		if (empty($function_name)) {
			$smarty->trigger_error("SmartySession: missing 'func' parameter", E_USER_NOTICE);
			return;
		}
		...
		$classReflector = new ReflectionClass($this->Session);
			...
			foreach ($funcParams as $param) {
				...
				if (isset($processedParams[$paramName])) {
					...
					if ($param->isDefaultValueAvailable()) {
						...
						$smarty->trigger_error("SmartySession: Error ".$paramName." parameter is required for method ".$function_name, E_USER_NOTICE);
					} else {
		...
		if (count($arrayParams)) {
			...
			$smarty->trigger_error("SmartySession: Error " . $classReflector->name . "::" . $function_name . " is not defined", E_USER_NOTICE);
			return;
		}

		if ($show_call) {
			echo '<pre>SmartySession calling $session->' . $function_name . ' with these parameters: <br />';
			var_dump($parameters);
			echo '</pre>';
		}

		$result = call_user_func_array(array($this->Session,$function_name),$parameters);
