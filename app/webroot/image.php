...
define('CACHE_DIR',				'images/radiotime/50x50/');
define('DOCUMENT_ROOT',			$_SERVER['DOCUMENT_ROOT']);


/**
 * Class ImageComponent
 * handles image caching / resizing
 */
...
class Image{
	 ...
	 */
	function resize($image_url, $w, $h, $color=null, $cropratio=null, $quality=null, $nocache=false){
		if (!isset($image_url))
		{
		...
		// default image for missing or bad images
		$default_image = DOCUMENT_ROOT.'/'.CACHE_DIR.'/default.png'; 
		
		// Images must be local files, so for convenience we strip the domain if it's there
		$image			= preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $image_url);

		$image_to_resize = DOCUMENT_ROOT.'/'.CACHE_DIR.$image;
		...
		$imagefile = $image_to_resize;

		if (!file_exists($imagefile))
		{
			/*
			header('HTTP/1.1 404 Not Found');
			echo 'Error: image does not exist: ' . $imagefile;
			exit();
			*/
			self::save_image($image_url, DOCUMENT_ROOT.'/'.CACHE_DIR.$image);
		}
		...
		$maxWidth		= (isset($w)) ? (int) $w : 0;
		$maxHeight		= (isset($h)) ? (int) $h : 0;
		...
		$resized		= $docRoot . '/' . CACHE_DIR . $resizedImage;
		...
		$src	= $creationFunction($imagefile);
...
// do the resize
$image = new Image();
$image->resize($_REQUEST['src'], $_REQUEST['w'], $_REQUEST['h']);
